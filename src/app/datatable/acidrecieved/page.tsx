...
export default async function Datatable() { 
  ...
  const result = await db('acidrecieved')
    .select('*')
  ...
  return (
    <>
      <h1 className="font-bold">Data Table HCI Fill</h1>
        ...
        <table className="min-w-full">
          <thead>
            <tr className="text-left border-b">
              ...
              <th>volume_T1_Kg</th>
              <th>volume_T2_Kg</th>
              <th>before_volume_T1_Kg  </th>
              <th>before_volume_T2_Kg</th>
          ...
          <tbody>
            {result.map((row, index) => (
              <tr key={index} className="border-b">
                ...
                <td>{row.volume_T2_Kg}</td>
                <td>{row.before_volume_T1_Kg}</td>
                <td>{row.before_volume_T2_Kg}</td>
                {/* <td>{row.volume_T1_Kg}</td>
                <td>{row.volume_T1_Kg}</td> */}
